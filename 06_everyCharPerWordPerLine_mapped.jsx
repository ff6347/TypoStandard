...
for(var i = 0;i < theTextFrame.lines.length; i++){
	...
	for(var j = 0;j < theTextFrame.lines.item(i).words.length ;j++){
		// this loops thru all characters in all word in everyline
		for (var k = 0; k < theTextFrame.lines.item(i).words.item(j).characters.length;k++){
				// this mappes the values
				var theValue = (100 / theTextFrame.lines.item(i).words.item(j).characters.length) *k;
				theTextFrame.lines.item(i).words.item(j).characters.item(k).fillTint = theValue;
			}
		}
	}
}